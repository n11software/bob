...
extern "C" __attribute__((ms_abi)) unsigned long long boot(void* ImageHandle, struct SystemTable* SystemTable) {
    ...
    void* KernelBaseAddress = (void*)-1;
    unsigned long long KernelSize = 0;
    ...
    for (ELFProgramHeaders* ProgramHeader=ProgramHeaders;(char*)ProgramHeader<(char*)ProgramHeaders+KernelHeaders.ProgramHeaderEntries*KernelHeaders.ProgramHeaderEntrySize;ProgramHeader=(ELFProgramHeaders*)((char*)ProgramHeader+KernelHeaders.ProgramHeaderEntrySize)) {
        switch (ProgramHeader->Type) {
            case 1: {
                if (KernelBaseAddress > (void*)ProgramHeader->PhysicalAddress) {
                    KernelBaseAddress = (void*)ProgramHeader->PhysicalAddress;
                }
                if (KernelSize < ProgramHeader->PhysicalAddress + ProgramHeader->MemorySize) {
                    KernelSize = ProgramHeader->PhysicalAddress + ProgramHeader->MemorySize;
                }
    ...
    BOB* bob;
    CheckStatus SystemTable->BootServices->AllocatePool(2, sizeof(BOB), (void**)&bob);
    if (bob == nullptr) {
        PrintTGA(ErrorIcon, SystemTable, framebuffer, framebuffer.Width/2, framebuffer.Height/2);
        while(1) asm("hlt");
    }
	...
	unsigned long long MapSize, MapKey;
	unsigned long long DescriptorSize;
	unsigned long long DescriptorVersion;
	{
		
		SystemTable->BootServices->GetMemoryMap(&MapSize, Map, &MapKey, &DescriptorSize, &DescriptorVersion);
		SystemTable->BootServices->AllocatePool(4, MapSize, (void**)&Map);
		SystemTable->BootServices->GetMemoryMap(&MapSize, Map, &MapKey, &DescriptorSize, &DescriptorVersion);
    ...
    bob->RSDP = rsdp;
    bob->FontFile = buffer;
    bob->KernelAddress = KernelBaseAddress;
    bob->KernelSize = KernelSize;

    SystemTable->BootServices->ExitBootServices(ImageHandle, MapKey);

	KernelBaseAddress = (void*)KernelHeaders.Entry;
    KernelSize = KernelSize - (unsigned long long)KernelBaseAddress;
